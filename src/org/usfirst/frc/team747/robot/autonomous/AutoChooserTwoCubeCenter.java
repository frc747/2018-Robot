...
public class AutoChooserTwoCubeCenter extends CommandGroup {
	public AutoChooserTwoCubeCenter() {

		if (Robot.gameData.length() > 0) {
			if (Robot.gameData.charAt(0) == 'L') {
				// Two Cube Auto for Left Side
			    addSequential(new CenterLeftSideSwitchTwoCube());
			    SmartDashboard.putString("GameDataValue", "TwoCubeLeftAuto");
			    
			} else if (Robot.gameData.charAt(0) == 'R') {
				// Two Cube Auto for Right Side
			    addSequential(new CenterRightSideSwitchTwoCube());
